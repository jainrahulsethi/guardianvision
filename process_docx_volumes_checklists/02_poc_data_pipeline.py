...
# MAGIC %pip install -qqqq -U pypandoc_binary==1.13 databricks-vectorsearch transformers==4.41.1 torch==2.3.0 tiktoken==0.7.0 langchain-text-splitters==0.2.0  mlflow mlflow-skinny
...
# Set the parameters
mlflow.log_params(_flatten_nested_params({"data_pipeline": docx_data_pipeline_config}))
...
mlflow.log_dict(docx_data_pipeline_config, "data_pipeline_config.json")
...
tag_delta_table(destination_tables_config["raw_files_table_name"], docx_data_pipeline_config)
...
tag_delta_table(destination_tables_config["parsed_docs_table_name"], docx_data_pipeline_config)
...
tag_delta_table(destination_tables_config["chunked_docs_table_name"], docx_data_pipeline_config)
...
tag_delta_table(destination_tables_config["vectorsearch_index_name"], docx_data_pipeline_config)
...
index.similarity_search(columns=["chunked_text", "chunk_id", "path"], query_text="Safety checklist for Construction activity with scaffold and heavy machinery operation")
